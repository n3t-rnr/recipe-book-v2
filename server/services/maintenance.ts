 ...
 * Today it purges recipes that have been in the trash for TRASH_DAYS (F-08); the backup (F-40)
 * and the image cleanup (Kap. 4.6) join this run in their milestones.
 */
export type MaintenanceDeps = TrashDeps & { state: Pick<RuntimeState, 'db'> };
...
/**
 * One maintenance run. Never throws: a failure is logged and the next run tries again.
 * Skipped in read-only mode (NF-19), where every write would fail anyway.
 */
export function runMaintenance(deps: MaintenanceDeps): void {
  ...
  try {
    const purged = purgeExpired(deps);
    if (purged > 0) deps.log.info('maintenance', { trashPurged: purged });
  } catch (err) {
    deps.log.error('maintenance failed', { task: 'trash', err });
  }
}
